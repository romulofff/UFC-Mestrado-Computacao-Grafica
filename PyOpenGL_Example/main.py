...
class camera:
    def __init__(self, point_xyz, lookat,view_up, sagital_point_xyz):
        ...
        self.vector_j = normalize_vector(cross_product(self.vector_k,self.vector_i))
        self.world_to_camera = np.matrix([[ self.vector_i[0,0], self.vector_i[0,1], self.vector_i[0,2], -(dot(self.vector_i,self.point_xyz)) ],
                                         [ self.vector_j[0,0], self.vector_j[0,1], self.vector_j[0,2], -(dot(self.vector_j,self.point_xyz)) ],
                                         [ self.vector_k[0,0], self.vector_k[0,1], self.vector_k[0,2], -(dot(self.vector_k,self.point_xyz)) ],
                                         [ 0,0,0,1]]
                                         )
...
view = camera(point_xyz=np.matrix([10,10,10]), lookat=np.matrix([2,0,0]), view_up=np.matrix([0,2,0]), sagital_point_xyz=np.matrix([2,2,2]))
